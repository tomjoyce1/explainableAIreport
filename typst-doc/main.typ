...
== Psychological and behavioral impacts - HCI
...
In the early stages of interaction, users often display overtrust, placing excessive confidence in AI systems due to their coherent and authoritative communication style. When hallucinations occur, this trust can be violated, resulting in frustration or feelings of betrayal. According to @Massenon2025, hallucination-related reviews were associated with strong negative sentiment (mean VADER (Valence Aware Dictionary and sEntiment Reasoner) score = −0.65) and lower app ratings (approximately 1.8/5) compared with general AI reviews (around 3.9/5), demonstrating the erosion of user trust once errors become apparent.
\

With repeated exposure, users may develop undertrust, becoming overly skeptical and distrusting even accurate outputs. Bucina et al. @Bucinca2021 relate this pattern to the lack of design mechanisms that promote careful reflection. Their discussion of “cognitive forcing functions” as interventions highlights how small design frictions could encourage users to pause and verify information before acting on it. Without such mechanisms, users alternate between inattentive overreliance and blanket rejection of AI outputs.
\
...
\ 
Another method that complements this, @Su2024 propose a real-time framework that is unsupervised (MIND) for detecting hallucinations. This is done using the internal state of LLMs during text generation. It uses a binary classifier to spot signs of hallucination during text generation using internal model signals across different models and domains. When tested against human-annotated benchmarks, this approach outperformed previous detection techniques. This method can be scaled and is model-agnostic, so it can potentially be used in many AI systems.
