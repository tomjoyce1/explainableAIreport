...
Researchers have developed taxonomies of error types specific to AI-generated content and have begun creating coding schemes validated through empirical data such as error reports from users interacting with large language models to better characterize and quantify these harms (@Naiseh2021). However these efforts highlight the difficulty of standardizing such evaluations as the current taxonomies are often subjective or limited to isolated domains. This puts urgent need for generalized frameworks that can categorize and measure hallucination harm across contexts (@Bender2021).
